...
//CE_Lib::redirectPage($_SERVER['PHP_SELF'].'?fuse=reports&amp;report=Upcoming+Charges&amp;controller=index&amp;type=Income&amp;view=viewreport&sortbydate='.(($sortbydate)? 1 : 0));
        ...
        while (list($id, $customerid, $amount, $quantity, $amount_percent, $nextbilldate, $appliestoid, $disablegenerate, $subscription_id, $billingtype, $taxable, $late, $paymenttype, $DomainName) = $result->fetch()) {
            ...
            if ($taxrate > 0) {
                ...
                if ($taxable) {
                    $tax = $currency->format($user->getCurrency(), $amount * $quantity * $taxrate / 100);
                }
            }

            $masterGroup[] = array($customerid, $date, $amount * $quantity, $disablegenerate, $paymenttype, $tax, ($amount * $quantity + $tax), $id, $late, $appliestoid, $DomainName);
            if ($late) {
                $lateEntries[] = $id;
            }
        }
        ...
        if ($this->showOptionsForOverdueCharges
            ...
            if (!isset($this->lastPaidInvoiceInfo[$id])) {
                $queryGetLastPaidInvoiceInfo = "SELECT i.id, i.billdate, i.datepaid "
                        ...
                        ."               FROM invoiceentry ie "
                        ."               WHERE ie.recurringappliesto = ". $this->db->escape($id) ." "
                        ."              ) "
