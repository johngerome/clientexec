...
abstract class Exporter extends Database
{
    ...
    protected $_columns = array(
        ...
        'hosting' => array('id', 'clientid', 'username', 'plan', 'regdate', 'domain', 'status', 'nextinvoicedate', 'paymentterm', 'price', 'server', 'coupon', 'subscription_id'),
...
class Blesta_Exporter extends Exporter
{
        ...
        foreach ($allPackages as $p) {
            ...
            if ($package->groups[0]->type == 'standard') {
                ...
                foreach ($package->pricing as $price) {
                    $billingcycle = $this->cycle2ce($price->term, $price->period);

                    if ($billingcycle == '0m') {
                        $pricing[$billingcycle] = array(
                            'price' => $price->price
                        );
                    } else {
                        $pricing[$billingcycle] = array(
        ...
        foreach ($allPackages as $p) {
            ...
            if ($package->groups[0]->type == 'addon') {
                ...
                foreach ($package->pricing as $price) {
                    ...
                    //Clientexec allows to have only one setup fee for all the billing cycles in an addon option
                    //To avoid lossing income, this importer will be using the greatest value in setup fees from Blesta to Clientexec
                    if ($setup == '' || $setup < $price->setup_fee) {
                        $setup = $price->setup_fee;
                    }
                }
    ...
    public function exportPackagesGroups ()
    {
        ...
        do {
            $query = "SELECT pg.`id`, {$nameField}, {$descriptionField} FROM `package_groups` pg {$join} WHERE pg.`type` = 'standard' AND pg.`company_id` = {$this->companyId} LIMIT {$offset}, {$this->_mysqlBufferLimit}";
            $result = $this->query($query);
