function CustomFieldsSetup(options) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.data.customfieldchangable == 2) {
                    ...
                    if (response.data.customfieldclientchangable == 1) $('#formCustomFieldInfo').append('<input class="unchangeable_customfield" type="hidden" name="customfieldclientchangable" value="' + response.data.customfieldclientchangable + '" />');
                    ...
                    if (response.data.usedbyplugin != "") {
                        $('#fromplugin').html("<div class='alert alert-warning'>" + clientexec.lang("Custom field created by plugin:") + " <b>" + response.data.usedbyplugin + "</b></div>");
                    } else {
                        $('#fromplugin').html("");
                    }
    ...
    this._initializeTree = function() {
        this.dom.divCustomFieldTree.jstree({
            ...
            themes: {
                theme: 'default',
                url: '../templates/admin/js/jquery-jstree/themes/default/style.css',
                dots: true,
                icons: true
            },
            json_data: {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ce.msg(clientexec.lang('Custom fields reordered successfully'));
                    } else {
    ...
    this._setHandlers = function() {
        ...
        this.dom.buttonAdd.click(function() {
            RichHTML.msgBox(
                clientexec.lang('Enter the name of the new custom field'), {
                    type: 'prompt'
                },
                function(result) {
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                ...
                                if (response.success) {
                                    ...
                                    ce.msg(result.elements.value + clientexec.lang(' added to custom fields.'));
                                } else {
                                    ce.parseActionResponse(response);
                                }
                            }
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        ce.msg(that.inputs[that.settings.fieldName].val() + clientexec.lang(' saved successfully.'));
        ...
        this.dom.buttonDelete.click(function() {
            RichHTML.msgBox(
                clientexec.lang('Are you sure you want to delete ') + that.inputs[that.settings.fieldName].val() + ' ?', {
                    type: 'confirm',
                    buttons: {
                        confirm: {
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                if (response.success) {
                                    ...
                                    ce.msg(clientexec.lang(that.inputs[that.settings.fieldName].val() + ' successfully deleted.'));
                                    that.clearInputs();
                ...
                case 'INPUT':
                    switch (this.inputs[e].attr('type')) {
                        ...
                        case 'hidden':
                            ...
                            break;
                        case 'checkbox':
                            this.inputs[e].prop('checked', false);
                            break;
                    }
                    break;
                case 'TEXTAREA':
    ...
    this._showLoading = function() {
        ...
        this.dom.divLoading.css({
            'background': 'url("../templates/admin/images/loading.gif") no-repeat center center',
