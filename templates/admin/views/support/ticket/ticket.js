...
ticketview.actionstodoonload = function(ticketid) {
    ...
    $('#maincenter').addClass('with-top-view-bar');
...
ticketview.updateticket_view = function(metadata) {
    ...
    if (metadata.rating) {
        ...
        if (metadata.rating_text) {
            $('.ticket-top-bar-rating .rating-rate').attr('title', clientexec.lang("Comments") + (new Array(61).join('&nbsp;')));
...
ticketview.submitTicket = function(event, ticketstatus, closed) {
    ...
    if (fileBlobs.length == 0) {
        ...
        $('#SupportForm').fileupload('send', {
            ...
            var err = clientexec.lang("There was an error with this operation");
            try {
...
ticketview.itemsFilledOut = function(form) {
    strAlertMessage = clientexec.lang("You must complete all the fields before proceeding");
...
ticketview.bindtickettabs = function() {
    ...
    $('body').on('click', '#btnDeleteTicket', function() {
        RichHTML.msgBox(clientexec.lang("Are you sure you want to delete this ticket?"), { type: 'yesno' }, function(e) {
            if (e.btn === clientexec.lang("Yes")) {
                $.ajax({
...
ticketview.markTicketAsSpam = function() {
    RichHTML.msgBox(clientexec.lang("Are you sure you want to mark this ticket as spam?"), { type: 'yesno' }, function(e) {
        if (e.btn === clientexec.lang("Yes")) {
    ...
    if (!ticketview.subscribe_value) {
        ...
        $.ajax({
            ...
            success: function(responseObj) {
                ...
                $('#btnSubscribe').text(clientexec.lang("Unsubscribe"));
                ce.msg(clientexec.lang('You have subscribed to this ticket'));
                clientexec.update_ticket_filters();
        ...
        $.ajax({
            ...
            success: function(responseObj) {
                ...
                $('#btnSubscribe').text(clientexec.lang("Subscribe"));
                ce.msg(clientexec.lang('You have unsubscribed from this ticket'));
                clientexec.update_ticket_filters();
                //clientexec.pluginMgr.process("ticketfilters");
            }
        });
...
ticketview.add_new_metalog_message = function(log_data) {
    ...
    newMetaLog.children('.newstate').html(log_data.newstate == 'himself' ? clientexec.lang('himself') : log_data.newstate);
...
function ShowEditBox(id) {
    ...
    ticketview.editedOldMessage = $('#' + id).html();
    var regexp = /(\n)/gm;
    ...
    document.getElementById('editLinks_' + id).innerHTML = "<a style='font-size:11px;' href=\"javascript:SaveEditMessage('" + id + "');\">" + clientexec.lang('Save') + "</a>&nbsp;&nbsp;<a style='font-size:11px;' href=\"javascript:CancelEditMessage('" + id + "');\">" + clientexec.lang('Cancel') + "</a>";
...
function CancelEditMessage(id) {
    ...
    document.getElementById('editLinks_' + id).innerHTML = "<a style='font-size:11px;' href=\"javascript:ShowEditBox('" + id + "');\">" + clientexec.lang('Edit') + "</a>&nbsp;&nbsp;<a style='font-size:11px;' href=\"javascript:DeleteLog('" + id + "');\">" + clientexec.lang('Delete') + "</a>";
}
...
function SaveEditMessage(log) {
    ...
    $.ajax({
        ...
        success: function(t) {
            ...
            document.getElementById('editLinks_log_' + logid).innerHTML = "<a style='font-size:11px;' href=\"javascript:ShowEditBox('log_" + logid + "');\">" + clientexec.lang('Edit') + "</a>&nbsp;&nbsp;<a style='font-size:11px;' href=\"javascript:DeleteLog('log_" + logid + "');\">" + clientexec.lang('Delete') + "</a>";
        }
    });
...
function DeleteLog(log) {
    ...
    RichHTML.msgBox(clientexec.lang(text), { type: 'yesno' }, function(e) {
        if (e.btn === clientexec.lang("Yes")) {
            if (deletetag == 0) {
                $.ajax({
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            $.each(response.options, function(index, obj) {
                ...
                if (obj.department) {
                    $(li).html("<a href='javascript:void(0);' data-id='" + obj.assigneeId + "' onclick='ticketview.changeassignee(this);'><strong>" + clientexec.lang("Dept.") + " " + obj.assigneeLabel + "</strong></a>");
                } else {
            ...
            else $('.ticket-top-bar-assignedto .dropdown-toggle').text(clientexec.lang('Unassigned'));
...
ticketview.add_other_viewers = function() {
    ...
    if (usersViewingSamePage !== "") {
        usersViewingSamePage = clientexec.lang("Others viewing: ") + usersViewingSamePage.substring(0, usersViewingSamePage.length - 2);
